...
def render_dashboard():
    ...
    if actives:
        ...
        st.dataframe(
            df_act[["matricule", "nom_complet", "type_pause", "heure_debut"]],
    ...
    if all_logs:
        ...
        dfl["type_pause"] = dfl["type_pause"].map(PAUSE_LABELS)
        st.dataframe(
            dfl[["matricule", "nom_complet", "type_pause", "date_jour",
                 "heure_debut", "heure_fin", "duree_minutes", "statut"]],
            use_container_width=True, hide_index=True,
        )
        st.download_button(
...
def render_user_management():
    ...
    with st.expander("📥 Import en masse d'utilisateurs (CSV / Excel)", expanded=False):
        st.caption(
            "Le fichier doit contenir ces colonnes (dans n'importe quel ordre) : "
            "**Matricule**, **Nom**, **Mot de passe**, **Rôle** (Agent ou Admin)."
        )
        template = pd.DataFrame({
            "Matricule":    ["CN00001", "CN00002"],
            "Nom":          ["Agent Exemple Un", "Agent Exemple Deux"],
            "Mot de passe": ["motdepasse1", "motdepasse2"],
            "Rôle":         ["Agent", "Agent"],
        })
        st.download_button(
        ...
        fichier_import = st.file_uploader(
            "Importer le fichier complété", type=["csv", "xlsx"], key="bulk_import"
        )
        if fichier_import is not None:
            try:
                if fichier_import.name.endswith(".csv"):
                    df_import = pd.read_csv(fichier_import, dtype=str)
                else:
                    df_import = pd.read_excel(fichier_import, dtype=str)
            ...
            if df_import is not None:
                ...
                for col in df_import.columns:
                    cl = col.lower().replace(" ", "_").replace("é", "e").replace("ô", "o")
                    if cl in ("matricule",):
                        rename_map[col] = "matricule"
                    elif cl in ("nom", "nom_complet", "nom_agent"):
                        rename_map[col] = "nom_complet"
                    elif cl in ("mot_de_passe", "password", "mot_passe", "mdp"):
                        rename_map[col] = "mot_de_passe"
                    elif cl in ("role", "rôle", "profil"):
                        rename_map[col] = "role"
                df_import = df_import.rename(columns=rename_map)

                st.markdown(f"**{len(df_import)} ligne(s) détectée(s).** Aperçu :")
                st.dataframe(df_import.head(5), use_container_width=True, hide_index=True)

                if st.button("✅ Confirmer l'import en masse", type="primary"):
                    rows = df_import.fillna("").to_dict("records")
                    crees, ignores, erreurs = db.bulk_add_users(rows)
                    st.success(f"{crees} utilisateur(s) créé(s) avec succès.")
                    if ignores:
                        st.warning(f"{ignores} ligne(s) ignorée(s).")
                    for e in erreurs:
                        st.caption(f"⚠️ {e}")
                    st.rerun()
...
def main():
    ...
    st.sidebar.caption(f"💾 Base : `{db.DB_PATH}`")
    logout_button()
