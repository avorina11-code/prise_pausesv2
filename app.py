...
def agent_interface(user):
    ...
    st.caption(f"Matricule : {user['matricule']} — {date.today().strftime('%d/%m/%Y')}")
    ...
    if active_pause:
        debut = datetime.fromisoformat(active_pause["heure_debut"])
        ecoule = round((datetime.now() - debut).total_seconds() / 60.0, 1)
...
def render_dashboard():
    ...
    c2.metric("Agents en pause maintenant", nb_actives)
    c3.metric("Taux de simultanéité", f"{taux}%", delta=f"Seuil {seuil_pct}%")
    c4.metric("Statut seuil", "🔴 Dépassé" if taux > seuil_pct else "🟢 OK")

    if taux > seuil_pct:
        st.error(f"⚠️ ALERTE VIGIE : le seuil de simultanéité de {seuil_pct}% est franchi "
                  f"({nb_actives}/{effectif_total} agents en pause).")
    ...
    if all_logs:
        ...
        st.download_button(
            ...
            file_name=f"pause_logs_export_{date.today().isoformat()}.csv",
            mime="text/csv",
        )
    else:
...
def render_cold_analysis():
    ...
    jour_analyse = st.date_input("Date à analyser (logs internes)", value=date.today())
...
def render_config():
    ...
    with st.form("config_form"):
        ...
        seuil_pct = st.number_input("Seuil de simultanéité (%)", min_value=1, max_value=100,
                                     value=int(float(cfg.get("seuil_simultaneite_pct", 15))))
        ...
        if ok:
            ...
            db.set_config("seuil_simultaneite_pct", seuil_pct)
