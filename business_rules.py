    ...
    if type_pause == "Besoin":
        ...
        if occurrences >= max_occurrences or duree_cumulee >= max_duree:
            return _verdict(True, "danger", occurrences, max_occurrences, duree_cumulee, max_duree,
                             ...
                             "alerte envoyée à la Vigie, veuillez avisez directement la Vigie / Manager")
...
def evaluate_simultaneity(type_pause: str) -> dict:
    """
    Vérifie si déclencher une nouvelle pause dépasserait le seuil de simultanéité
    configuré pour ce type de pause.

    Chaque type a son propre seuil (configurable depuis l'espace Admin) :
    - Pause Déjeuner  → seuil_simultaneite_dejeuner_pct (défaut 15%)
    - Pause Besoin    → seuil_simultaneite_besoin_pct   (défaut 10%)
    - Personnelle / Autre → seuil Déjeuner utilisé par défaut

    Pour 'Besoin' : on ne bloque JAMAIS (exception légale), on alerte seulement.
    Pour les autres types : le bouton est désactivé si le seuil est déjà atteint.
    """
    effectif_total = int(db.get_config("effectif_total", 100))

    # Choix du seuil selon le type de pause
    if type_pause == "Besoin":
        seuil_pct = float(db.get_config("seuil_simultaneite_besoin_pct", 10))
    else:
        seuil_pct = float(db.get_config("seuil_simultaneite_dejeuner_pct", 15))

    seuil_absolu = max(1, int(effectif_total * seuil_pct / 100))

    # On ne compte que les pauses du même type pour la simultanéité
    actives = db.get_all_active_pauses()
    nb_en_pause = len([a for a in actives if a["type_pause"] == type_pause])
    ...
    return {
        ...
        "message": (
            f"Seuil de simultanéité {type_pause} franchi : "
            f"{nb_en_pause}/{effectif_total} agents en pause {type_pause} "
            f"({taux_actuel_pct}% > {seuil_pct}%)."
            if seuil_franchi else
            f"Simultanéité {type_pause} OK : {nb_en_pause}/{effectif_total} "
            f"agents en pause {type_pause} ({taux_actuel_pct}% / seuil {seuil_pct}%)."
        ),
    }
