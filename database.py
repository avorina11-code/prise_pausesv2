...
def add_user(matricule, nom_complet, plain_password, role):
    with get_cursor() as cur:
        cur.execute(
            """INSERT INTO users (matricule, nom_complet, password_hash, role, actif, date_creation)
               VALUES (?, ?, ?, ?, 1, ?)""",
            (matricule, nom_complet, hash_password(plain_password), role, datetime.now().isoformat()),
        )
    ...
    with get_cursor() as cur:
        cur.execute(
            """INSERT INTO pause_logs (matricule, type_pause, heure_debut, date_jour, statut)
               VALUES (?, ?, ?, ?, 'En cours')""",
            (matricule, type_pause, now.isoformat(), date.today().isoformat()),
        )
...
def end_pause(matricule):
    ...
    with get_cursor() as cur:
        ...
        if not row:
            return None
        debut = datetime.fromisoformat(row["heure_debut"])
        fin = datetime.now()
