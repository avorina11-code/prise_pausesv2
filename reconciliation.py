...
def _safe_parse_datetime(value):
    ...
    Retourne un objet datetime ou None si :
      - la valeur n'est pas interprétable comme horodatage, OU
      - les minutes ne tombent pas sur une tranche de 30 min (0 ou 30).

    Règle métier Vocalcom Mvola AE : les colonnes de la matrice sont espacées de
    30 minutes exactement (7:00 → 7:30 → 8:00 …). Toute colonne hors de cette
    grille (ex: 7:05, 7:15, artefact de format) est ignorée silencieusement.
    """
    try:
        ...
        try:
            ...
            return None  # Pas un horodatage du tout

    dt = parsed.to_pydatetime() if hasattr(parsed, "to_pydatetime") else parsed

    # Validation : minutes doivent être 0 ou 30 (tranche de 30 min), secondes ignorées
